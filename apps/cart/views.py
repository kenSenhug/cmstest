...
from apps.cart.serialziers import CartAddSerializer, CartDeleteSerializer, CartSKUSerializer
...
class CartView(APIView):
    ...
    def get(self, request):
        ...
        strict_redis = get_redis_connection('cart')  # type: StrictRedis
        # 读取购物车的商品数据，得到一个字典
        dict_data = strict_redis.hgetall('cart_%s' % user_id)  # {1: 2, 2: 2}  --> bytes
        # 读取商品勾选状态，得到一个列表
        list_data = strict_redis.smembers('cart_selected_%s' % user_id)  #  (1, )  --> bytes
    ...
    def put(self, request):
        ...
        strict_redis = get_redis_connection('cart')  # type: StrictRedis
        # 修改商品数量
        strict_redis.hset('cart_%s' % user_id, goods_id, count)
        # 修改商品的勾选状态
        if selected:
            strict_redis.sadd('cart_selected_%s' % user_id, goods_id)
        else:
            strict_redis.srem('cart_selected_%s' % user_id, goods_id)
        # 响应序列化数据
        return Response(s.data)
    ...
    def delete(self, request):
        ...
        strict_redis = get_redis_connection('cart')  # type: StrictRedis
        # 删除商品  hdel cart_1 1
        strict_redis.hdel('cart_%s' % user_id, goods_id)
